...
if __name__ == '__main__':
    ...
    demo_ins.logger().info(f"填充后：{res_text}")
    demo_ins.logger().info(f"去填充前：{res_text}")
    text = demo_ins.aes_unpad(res_text)
    demo_ins.logger().info(f"去填充后：{text}")
    # AES加密解密
    demo_ins.logger().info(f"\n")
    demo_ins.logger().info(f"加密解密")
    ...
    demo_ins.logger().info("ECB模式加密")
    demo_ins.logger().info(f"明文：{content}")  # 加密明文，bytes类型
    en_text = demo_ins.aes_ecb_encryption(content, password)
    demo_ins.logger().info(f"加密后：{en_text}")  # 加密明文，bytes类型
    demo_ins.logger().info("ECB模式解密")
    demo_ins.logger().info(f"密文：{en_text}")
    demo_ins.aes_ecb_decryption(en_text, password)
    demo_ins.logger().info(f"解密后：{content}")
    iv = b'1234567812345678'  # iv偏移量，bytes类型
    demo_ins.logger().info("CBC模式的加密")
    demo_ins.logger().info(f"明文：{content}")  # 加密明文，bytes类型
    en_text = demo_ins.aes_cbc_encryption(content, password, iv)
    demo_ins.logger().info(f"加密后：{en_text}")  # 加密明文，bytes类型
    demo_ins.logger().info("CBC模式解密")
    demo_ins.logger().info(f"密文：{en_text}")
    ...
    demo_ins.logger().info(f"中文明文：好好学习天天向上 需要gbk编码")
    content = "好好学习天天向上".encode('gbk')  # gbk编码，是1个中文字符对应2个字节，8个中文正好16字节
    demo_ins.logger().info(f"中文加密")
    demo_ins.logger().info(f"编码后明文：{content}")
    en_text = demo_ins.aes_ecb_encryption(content, password)
    demo_ins.logger().info(f"加密后：{en_text}")
    demo_ins.logger().info(f"中文解密")
    demo_ins.logger().info(f"密文：{en_text}")
    ...
    # 生成密钥对
    demo_ins.logger().info(f"生成密钥对")
    pubkey, prikey = demo_ins.rsa_get_key_pair()
    demo_ins.logger().info(f"公钥{pubkey}")
    demo_ins.logger().info(f"私钥{prikey}")
    content = 'Welcome to RSA'
    demo_ins.logger().info(f"明文：{content}")
    demo_ins.logger().info(f"加密：使用公钥")
    en_text = demo_ins.rsa_encryption(content.encode(), pubkey)
    demo_ins.logger().info(f"加密后{en_text}")
    demo_ins.logger().info(f"解密：使用私钥")
    demo_ins.logger().info(f"密文{en_text}")
    ...
    demo_ins.logger().info(f"本机ip地址: {local_ip_str}")
    ...
    demo_ins.logger().info(f"时间戳转为utc datetime对象")
    demo_ins.logger().info(f"{demo_ins.timestamp_int_to_utc_obj(1703829880)}")
    ...
    demo_ins.logger().info(f"\n")
    demo_ins.logger().info(f"移除一个字符串中的回车换行空格制表符等内容")
    temp_str = 'abc abc \n abc \r'
    ...
    # 将中文字符转为其拼音的首字母
    demo_ins.logger().info(f"\n")
    demo_ins.logger().info(f"将中文字符转为其拼音的首字母")
    demo_ins.logger().info(f"{Demo.get_pinyin('中国外汇交易中心')}")

    # 更加方便的日志记录工具
    demo_ins.logger().info(f"\n")
    demo_ins.logger().info(f"更加方便的日志记录工具")
    demo_ins.logger().info(f"{Demo.get_pinyin('中国外汇交易中心')}")
    ...
    # url编码解码
    demo_ins.logger().info(f"\n")
    demo_ins.logger().info(f"url编码解码")
    ...
    # 检测和转换编码方式
    demo_ins.logger().info(f"\n")
    demo_ins.logger().info(f"检测和转换编码方式")
    ...
    demo_ins.logger().info(f"本月节假日: {calendar_hashmap.get('holiday')}")
    demo_ins.logger().info(f"本月双休日: {calendar_hashmap.get('weekend')}")
    demo_ins.logger().info(f"本月工作日: {calendar_hashmap.get(0)}")
